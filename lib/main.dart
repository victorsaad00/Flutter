...
void _initDb(context) {
  ...
  //elemento da lista.
  Future<String> alunosData = DefaultAssetBundle.of(context).loadString('data/alunos.json');
  ...
  Future<String> professorData = DefaultAssetBundle.of(context).loadString('data/professor.json');
  professorData.then((value) {
      List<Map<String, dynamic>> jsonMaps = json.decode(value).cast<Map<String, dynamic>>();
      List<Professor> professor =
          jsonMaps.map<Professor>((json) => Professor.fromJson(json)).toList();
      professor.forEach((professor) => professorController.save(professor));
  });
